...
const Sidebar = ({
  ...
  useEffect(() => {
    ...
    const buildGroupedMenusFromPermissions = (
      perms: string[]
    ): MenuCategory[] => {
      ...
      if (
        perms.includes("bills") ||
        perms.includes("prepaid") ||
        perms.includes("postpaid")
      ) {
        const billsChildren: SubMenuItem[] = [];
        if (perms.includes("prepaid"))
          billsChildren.push({ title: "Prepaid", link: "/bills/prepaid" });
        if (perms.includes("postpaid"))
          billsChildren.push({ title: "Postpaid", link: "/bills/postpaid" });
        if (billsChildren.length > 0) {
          items.push({
            title: "Bills",
            icon: "/icons/bills.svg",
            hasSubmenu: true,
            submenu: billsChildren,
          });
        }
      }
        ...
        <div className="flex flex-col w-full h-fit overflow-hidden overflow-y-auto scrollbar-hide">
          <header
            className={` dark:bg-primary-dark flex justify-center border-b border-b-primary-border dark:border-dark-border items-center ${
              isSidebarCollapsed ? "bg-primary px-4" : "bg-white px-10"
            } py-6`}
          >
            <img
              src={
                        ...
                        {menuItem.hasSubmenu ? (
                          <div className="relative w-full">
                            <button
                              ...
                              className={`flex items-center gap-4 py-3 px-4 mb-1 text-sm cursor-pointer rounded-lg w-full text-left ${
                                ...
                                {menuItem.submenu?.map((subItem, subIndex) => (
                                  <li key={subIndex} className="relative">
                                    ...
                                    <button
                                      onClick={() =>
                                        subItem.link &&
                                        onNavigate?.(subItem.link)
                                      }
                                      className={`block ml-4 mr-4 py-3 px-4 pl-8 rounded-lg transition-all duration-200 w-full text-left text-sm relative z-10 ${
                          ...
                          <button
                            onClick={() => {
                              if (menuItem.title === "Logout" && onLogout) {
                                onLogout();
