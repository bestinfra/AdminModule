...
const getLayoutClass = (layout: LayoutType, gap?: string) => {
    const baseClass = layout === 'row' ? 'flex flex-row' : layout === 'column' ? 'flex flex-col' : 'grid';
                ...
                <div className={`grid ${gridClass} gap-4`}>
                    {rows.map((row, rowIndex) => {
                        ...
                        return (
                            <div 
                                key={rowIndex} 
                                className={`${rowLayoutClass} ${rowSpanClasses} ${row.className || ''}`}
                            >
                                {row.items.map((item, itemIndex) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="text-md font-bold text-TextSecondary">{item.title}</div>
