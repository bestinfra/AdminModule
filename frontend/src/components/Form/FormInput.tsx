...
const FormInput: React.FC<FormInputProps> = ({ 
  ...
  const { name, type, label, placeholder, required, options, className: inputClassName, colSpan = 1, icon, description, validation } = input;
  ...
  const commonProps: CommonInputProps = {
    ...
    onChange: (event: FormInputEvent) => {
      ...
      // Call individual onChange handler if provided
  ...
  const renderInput = () => {
    ...
    switch (type) {
        ...
        return (
          <Dropdown
            ...
            onChange={(e) => {
              ...
              const newValue = isMultiSelect ? e.target.value as string[] : e.target.value as string;
              onInputChange(name, newValue, input);
              input.onChange?.(newValue);
            }}
            options={options || []}
      ...
      case 'chosenfile':
        return (
          <DragDropFileUpload
            ...
            preview={fileSingleValue instanceof File ? URL.createObjectURL(fileSingleValue) : null}
            isDragOver={false}
      ...
      default:
        ...
        const allowedTextInputTypes = ['text','email','password','number','date','tel','url'] as const;
        const textInputType = allowedTextInputTypes.includes(type as any) ? type : 'text';
        return (
          <input
            {...commonProps}
            type={textInputType}
  ...
  return (
    <div className={`w-full ${gridSpanClass}`}>
      <div className="flex flex-col gap-2">
        {label && !['chosenfile', 'textareafield', 'radio', 'switch', 'checkbox'].includes(type) && (
        ...
        <div className="relative">
          ...
          {showError && error && (
            <span
              id={errorId}
              className="absolute top-0 left-6 -translate-y-1/2 z-20  bg-white text-red-500 text-sm font-medium"
              role="alert"
              aria-live="polite"
            >
              {error}
          ...
          {React.cloneElement(renderInput(), {
            className: `${renderInput().props.className || ''} ${showError ? 'border-red-500' : ''}`.trim()
          })}
        </div>
