...
const MeterDetails: React.FC = () => {
    ...
    useEffect(() => {
        ...
        fetch(`${BACKEND_URL}/meters/${meterId}/view`)
            .then(async (res) => {
                ...
                if (!result.success)
                    throw new Error(
                        result.message || 'Failed to fetch meter details'
                    );
                setMeter(result.data);
            })
            .catch((err) =>
                setError(err.message || 'Failed to fetch meter details')
            )
    ...
    if (loading)
        return <div className="p-8 text-center">Loading meter details...</div>;
    if (error)
