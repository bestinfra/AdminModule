...
const Step1: React.FC<Step1Props> = ({
    ...
    ], [valid.first_name, valid.last_name, valid.email, valid.phone, valid.id_type, valid.id_number, formData.id_type]);
    ...
    const validateIdNumber = useCallback((value: string, type: string): boolean => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {formInputs.map((input) => (
                    <div key={input.name} className="space-y-2">
                        ...
                        />
                        {errors[input.name] && (
                            <p className="text-red-500 text-xs">{errors[input.name]}</p>
                        )}
                    </div>
                ))}
            </div>
        </div>
    ), [formInputs, formData, errors, isFieldEnabled, handleFormInputChange, getFieldClasses]);
    ...
    ), [buildingSearch, setBuildingSearch, isFieldEnabled, getFieldClasses, valid.building, selectedBuilding, handleBuildingSelect]);
    ...
    ), [formData.property_type, formData.account_type, handleInputChange, setFieldFocus, isFieldEnabled, getFieldClasses, valid.property_type, valid.account_type]);
    ...
    ), [formData.move_in_date, handleInputChange, setFieldFocus, isFieldEnabled, getFieldClasses, valid.move_in_date]);
                                ...
                                columns: [
                                    {
                                        name: 'FormContainer',
                                        props: {
                                            children: <ErrorSummaryComponent />,
                                            className: 'mb-8'
                                        }
                                    }
                                ]
                                ...
                                columns: [
                                    {
                                        name: 'FormContainer',
                                        props: {
                                            children: <ConsumerInformationComponent />,
                                            className: 'mb-8'
                                        }
                                    }
                                ]
